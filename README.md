...
**Einordnung in die Reihe (die Kanten des Graphen):** CART ist die **Wurzel** der Linie. Er wählt Splits **gierig** und baut den Baum in einem Zug; daraus folgen die beiden Schwächen, die die übrigen Stücke antreiben – **Überanpassung** (ein weitergewachsener Baum lernt das Rauschen) und **Instabilität** (andere Stichprobe, anderer Baum).
...
|---|---|
| Flacher Baum (Tiefe 3, Blatt ≥ 5) | ✅ 8 Blätter, Trainingsfehler **15.8 %**, Testfehler **17.2 %** (Raten: 46.4 %), AUC 0.882, Log-Loss 0.42. Der erste Split ist das **Ladegewicht** (798 kg), danach Verkehr. |
...
| Rauschmerkmale | ⚠️ Der volle Baum benutzt sie: **16 %** seiner Splits (3 Rauschmerkmale) bzw. **29 %** (8) liegen auf Rauschen – fast alle tief im Baum (in den ersten vier Ebenen zusammen nur 5 bzw. 8 Splits über sechs Datensätze). |
...
| Gini gegen Entropie | ➖ Sie sind fast austauschbar: in **2 von 6** Datensätzen dieselben Merkmale in den ersten sieben Splits, die Vorhersagen (Tiefe 4, Blatt ≥ 5) stimmen zu **92 bis 98 %** überein. |
| Regression (Tiefe 4, Blatt ≥ 5) | ✅ 16 Blätter, Trainings-RMSE 12.3 min, Test-RMSE **14.4 min** gegen 27.2 min beim Mittelwert (R² 0.719); wichtigster Split Distanz (65.5 %). Der Baum liefert eine **Treppe**, keine Gerade – eine lineare Steigung braucht viele Blätter. |

## Was die Demo zeigt

- **CART in Aktion:** der Baum wächst Split für Split (Ebene für Ebene, wie CART ihn baut) mit Schritt-Regler und Abspielen: Baumdiagramm (Blätter nach Wert gefärbt, oranger Rand bei Splits auf Rauschmerkmalen, schwarzer Ring um den nächsten Split), die Vorhersage über zwei wählbare Merkmale (übrige auf dem Median), der **Weg einer Testlieferung** durch den Baum und die **Split-Suche** des nächsten Knotens (bester Gain je Merkmal, Gain über alle Schwellen des gewählten Merkmals).
...
- **Split-Suche:** je Merkmal sortieren, kumulative Summen (Klassenzahl bzw. Summe und Quadratsumme) liefern die Unreinheit beider Kinder für **alle** Schwellen in einem Zug; Kandidaten sind Mitten zwischen verschiedenen benachbarten Werten, zulässig nur mit Mindestblattgröße. Bei Gleichstand gewinnt das kleinste Merkmal, dann die kleinste Schwelle.
- **Unreinheit:** Gini 2p(1−p), Entropie (bit), Varianz; Blattwert = Anteil bzw. Mittelwert. Wachsen in **Breitenreihenfolge**, damit "der Baum nach k Splits" einfach der Baum ist, dessen Knoten ab dem k-ten inneren wieder Blätter sind.
...
- **Wurzel ist stabil, der Rest nicht:** ein dominantes Merkmal (das Ladegewicht bei der Klassifikation, die Distanz bei der Regression) macht den ersten Split eindeutig; die Instabilität sitzt in den unteren Ebenen.
- **Gleichstände sind echt:** in kleinen Knoten (zwei Lieferungen, oder wenige mit großer Mindestblattgröße) sind mehrere Splits gleich gut; scikit-learn wählt zufällig, diese Umsetzung das kleinste Merkmal. Deshalb sind die Bäume nur dort **exakt gleich** mit scikit-learn, wo es keine Gleichstände gibt (siehe Verifikation).
- **Treppen statt Geraden, Splits nur an einer Achse, keine Extrapolation:** siehe die Tabelle "Wo die Annahmen enden" in der App.

## Verifikation

`tests/test_algorithm.py` (37 Tests): Split-Suche gegen eine **Brute-Force-Suche** über alle Schwellen (alle drei Kriterien, mit Gleichständen und Mindestblattgröße); **scikit-learn** (`DecisionTreeClassifier/Regressor`) liefert bei stetigen Merkmalen und ohne Gleichstände **dieselben Bäume**, Vorhersagen und Blattwerte; die **Wichtigkeit** stimmt auf scikit-learns eigenem Baum überein; **Pfad der Kosten-Komplexität und beschnittene Bäume** stimmen mit `cost_complexity_pruning_path` und `ccp_alpha` überein (auf scikit-learns eigenen Bäumen, damit Gleichstände beim Wachsen nichts verdecken); Grenzfälle (Tiefe 0, `min_samples_leaf` = n, reine Knoten, konstantes Merkmal, voller Baum passt das Training); Breitenreihenfolge und Wachsen Split für Split; Gütemaße gegen `sklearn.metrics`.
...
| `cart_algorithm.py` | Split-Suche, Wachsen, Vorhersage, Wichtigkeit, Beschneiden, Gütemaße |
...
| `cart_visualization.py` | Baumdiagramm, Karte, Split-Suche, Kurven |
